 ...
 * are covered by the Common Public License 1.0 (http://opensource.org/licenses/cpl.php) which can
 * be found in the file CPL.TXT at the root of this distribution. By using this software in any
 * fashion, you are agreeing to be bound by the terms of this license. You must not remove this
...
public class Compiler implements Opcodes {
  ...
  // symbol->localbinding
  public static final Var LOCAL_ENV = Var.create(null);
  // vector<localbinding>
  public static final Var LOOP_LOCALS = Var.create();
  // Label
  public static final Var LOOP_LABEL = Var.create();
  // vector<object>
  public static final Var CONSTANTS = Var.create();
  // keyword->constid
  public static final Var KEYWORDS = Var.create();
  // var->constid
  public static final Var VARS = Var.create();
  // FnFrame
  public static final Var METHOD = Var.create(null);
  // null or not
  public static final Var IN_CATCH_FINALLY = Var.create(null);
  // String
  public static final Var SOURCE = Var.create("NO_SOURCE_FILE");
  // String
  public static final Var SOURCE_PATH =
      Var.intern(Namespace.findOrCreate(Symbol.create("clojure")), Symbol.create("*file*"), null);
  // Integer
  public static final Var LINE = Var.create(0);
  // Integer
  public static final Var LINE_BEFORE = Var.create(0);
  public static final Var LINE_AFTER = Var.create(0);
  // Integer
  public static final Var NEXT_LOCAL_NUM = Var.create(0);
  // Integer
  public static final Var RET_LOCAL_NUM = Var.create();
  // DynamicClassLoader
  public static final Var LOADER = Var.create();
  ...
  public static final IPersistentMap specials =
      PersistentHashMap.create(
          ...
          null,
          QUOTE,
          new ConstantExpr.Parser(),
          THE_VAR,
          new TheVarExpr.Parser(),
          DOT,
          new HostExpr.Parser(),
          ASSIGN,
          new AssignExpr.Parser(),
          //		TRY_FINALLY, new TryFinallyExpr.Parser(),
          TRY,
          new TryExpr.Parser(),
          THROW,
          new ThrowExpr.Parser(),
          MONITOR_ENTER,
          new MonitorEnterExpr.Parser(),
          MONITOR_EXIT,
  ...
  static final NilExpr NIL_EXPR = new NilExpr();
  static final BooleanExpr TRUE_EXPR = new BooleanExpr(true);
  static final BooleanExpr FALSE_EXPR = new BooleanExpr(false);
  ...
  static {
    ...
    for (int i = 0; i <= MAX_POSITIONAL_ARITY; ++i) {
      Type[] a = new Type[i];
      for (int j = 0; j < i; j++) a[j] = OBJECT_TYPE;
      ARG_TYPES[i] = a;
    }
  ...
  static Symbol resolveSymbol(Symbol sym) {
    ...
    if (sym.ns != null || sym.name.indexOf('.') > 0) return sym;
    Object o = currentNS().getMapping(sym);
    if (o == null) return Symbol.intern(currentNS().name.name, sym.name);
    else if (o instanceof Class) return Symbol.intern(null, ((Class) o).getName());
  ...
  private static Expr analyze(C context, Object form, String name) throws Exception {
    ...
    else if (fclass == String.class) return new StringExpr(((String) form).intern());
    ...
    else if (form instanceof IPersistentMap) return MapExpr.parse(context, (IPersistentMap) form);
    else if (form instanceof IPersistentSet) return SetExpr.parse(context, (IPersistentSet) form);
    ...
    if (op instanceof Symbol || op instanceof Var) {
      ...
      if (v != null) {
        ...
        if (ret != null) {
          ...
          if (arities == null || arities.contains(arity)) return ret;
        }
      }
    }
    return null;
  ...
  private static Expr analyzeSeq(C context, ISeq form, String name) throws Exception {
    ...
    try {
      ...
      if (inline != null) return analyze(context, inline.applyTo(RT.rest(form)));
      IParser p;
      ...
      if (!(e instanceof CompilerException))
        throw new CompilerException(
            String.format("%s:%d: %s", SOURCE.get(), (Integer) LINE.get(), e.getMessage()), e);
  ...
  public static Object resolveIn(Namespace n, Symbol sym) throws Exception {
    // note - ns-qualified vars must already exist
    if (sym.ns != null) {
      Namespace ns = Namespace.find(Symbol.create(sym.ns));
      if (ns == null) throw new Exception("No such namespace: " + sym.ns);
      Var v = ns.findInternedVar(Symbol.create(sym.name));
      if (v == null) throw new Exception("No such var: " + sym);
      ...
      if (o == null) throw new Exception("Unable to resolve symbol: " + sym + " in this context");
      return o;
    }
  }
  ...
  static Var lookupVar(Symbol sym, boolean internNew) throws Exception {
    ...
    if (sym.ns != null) {
      ...
      if (o == null) {
        ...
        if (internNew) var = currentNS().intern(Symbol.create(sym.name));
      } else if (o instanceof Var) {
        var = (Var) o;
      } else {
    ...
    if (b != null) {
      ...
      if (RT.get(method.locals, b) != null && IN_CATCH_FINALLY.get() != null) {
        method.localsUsedInCatchFinally =
            (PersistentHashSet) method.localsUsedInCatchFinally.cons(b.idx);
      }
    }
    return b;
  }
  ...
  public static Object loadFile(String file) throws Exception {
    ...
    try {
      return load(
          new InputStreamReader(f), new File(file).getAbsolutePath(), (new File(file)).getName());
    } finally {
      f.close();
    }
  }
  ...
  */
    ...
    try {
      Var.pushThreadBindings(
          RT.map(
              ...
              RT.CURRENT_NS, RT.CURRENT_NS.get(), SOURCE, "REPL"));
      w.write("Clojure\n");
  ...
  //
  //			return new IdenticalExpr(analyze(C.EXPRESSION, RT.second(form)), analyze(C.EXPRESSION,
  // RT.third(form)));
  //		}
  //	}
  // }
  ...
  abstract static class UntypedExpr implements Expr {
  ...
  static class DefExpr implements Expr {
    ...
    public Object eval() throws Exception {
      if (initProvided) var.bindRoot(init.eval());
      if (meta != null) {
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object form) throws Exception {
        ...
        if (RT.count(form) > 3) throw new Exception("Too many arguments to def");
        else if (RT.count(form) < 2) throw new Exception("Too few arguments to def");
        else if (!(RT.second(form) instanceof Symbol))
          throw new Exception("Second argument to def must be a Symbol");
        Symbol sym = (Symbol) RT.second(form);
        Var v = lookupVar(sym, true);
        if (v == null) throw new Exception("Can't refer to qualified var that doesn't exist");
        if (!v.ns.equals(currentNS())) {
          if (sym.ns == null)
            throw new Exception(
                "Name conflict, can't def "
                    + sym
                    + " because namespace: "
                    + currentNS().name
                    + " refers to:"
                    + v);
          else throw new Exception("Can't create defs outside of current ns");
        }
        IPersistentMap mm = sym.meta();
        mm =
            (IPersistentMap) RT.assoc(mm, RT.LINE_KEY, LINE.get()).assoc(RT.FILE_KEY, SOURCE.get());
  ...
  abstract static class LiteralExpr implements Expr {
    abstract Object val();
  ...
  public abstract static class HostExpr implements Expr, MaybePrimitiveExpr {
    ...
    public static void emitBoxReturn(FnExpr fn, GeneratorAdapter gen, Class returnType) {
      if (returnType.isPrimitive()) {
        if (returnType == boolean.class) {
          ...
          } else if (returnType == double.class) gen.invokeStatic(DOUBLE_TYPE, doubleValueOfMethod);
          else if (returnType == long.class) gen.invokeStatic(LONG_TYPE, longValueOfMethod);
          else if (returnType == byte.class) gen.invokeStatic(BYTE_TYPE, byteValueOfMethod);
          else if (returnType == short.class) gen.invokeStatic(SHORT_TYPE, shortValueOfMethod);
        }
      }
    }
    ...
    public static void emitUnboxArg(FnExpr fn, GeneratorAdapter gen, Class paramType) {
      if (paramType.isPrimitive()) {
        if (paramType == boolean.class) {
          ...
          if (paramType == int.class) m = intValueMethod;
          else if (paramType == float.class) m = floatValueMethod;
          else if (paramType == double.class) m = doubleValueMethod;
          else if (paramType == long.class) m = longValueMethod;
          else if (paramType == byte.class) m = byteValueMethod;
          else if (paramType == short.class) m = shortValueMethod;
    ...
    private static Class maybeClass(Object form, boolean stringOk) throws Exception {
      ...
      if (form instanceof Symbol) {
        ...
        {
          if (sym.name.equals("ints")) c = int[].class;
          ...
          else if (sym.name.equals("chars")) c = double[].class;
      ...
      } else if (stringOk && form instanceof String) c = RT.classForName((String) form);
      return c;
    }
    ...
    */
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object frm) throws Exception {
        ...
        // static target must be symbol, either fully.qualified.Classname or Classname that has been
        // imported
        ...
        if (maybeField) {
          ...
          if (c != null) maybeField = Reflector.getMethods(c, 0, sym.name, true).size() == 0;
          else if (instance != null && instance.hasJavaClass() && instance.getJavaClass() != null)
            maybeField =
                Reflector.getMethods(instance.getJavaClass(), 0, sym.name, false).size() == 0;
        }
        if (maybeField) // field
        {
          Symbol sym = (Symbol) RT.third(form);
          if (c != null) return new StaticFieldExpr(line, c, sym.name);
          else return new InstanceFieldExpr(line, instance, sym.name);
        } else {
          ISeq call =
              (ISeq) ((RT.third(form) instanceof ISeq) ? RT.third(form) : RT.rest(RT.rest(form)));
          ...
          if (c != null) return new StaticMethodExpr(line, c, sym.name, args);
          else return new InstanceMethodExpr(line, instance, sym.name, args);
        }
      }
    }
  }

  abstract static class FieldExpr extends HostExpr {}
  ...
  static class InstanceFieldExpr extends FieldExpr implements AssignableExpr {
    ...
    public InstanceFieldExpr(int line, Expr target, String fieldName) throws Exception {
      ...
      if (field == null && RT.booleanCast(RT.WARN_ON_REFLECTION.get())) {
        System.err.format(
            "Reflection warning, line: %d - reference to field %s can't be resolved.\n",
            line, fieldName);
      }
    }
    ...
    public void emitUnboxed(C context, FnExpr fn, GeneratorAdapter gen) {
      ...
      } else throw new UnsupportedOperationException("Unboxed emit of unknown member");
    }
  ...
  static class StaticFieldExpr extends FieldExpr implements AssignableExpr {
    static final Method getStaticFieldMethod =
        Method.getMethod("Object getStaticField(String,String)");
    static final Method setStaticFieldMethod =
  ...
  abstract static class MethodExpr extends HostExpr {
    ...
    public static void emitTypedArgs(
        FnExpr fn, GeneratorAdapter gen, Class[] parameterTypes, IPersistentVector args) {
  ...
  static class InstanceMethodExpr extends MethodExpr {
    ...
    public InstanceMethodExpr(int line, Expr target, String methodName, IPersistentVector args)
      ...
      if (method == null && RT.booleanCast(RT.WARN_ON_REFLECTION.get())) {
        System.err.format(
            "Reflection warning, line: %d - call to %s can't be resolved.\n", line, methodName);
      }
    }
    ...
    public Object eval() throws Exception {
      ...
      for (int i = 0; i < args.count(); i++) argvals[i] = ((Expr) args.nth(i)).eval();
      if (method != null) {
    ...
    public void emitUnboxed(C context, FnExpr fn, GeneratorAdapter gen) {
      ...
      if (method != null) {
        ...
        target.emit(C.EXPRESSION, fn, gen);
        if (!method.getDeclaringClass().isInterface()) gen.checkCast(type);
        ...
        Method m =
            new Method(methodName, Type.getReturnType(method), Type.getArgumentTypes(method));
        if (method.getDeclaringClass().isInterface()) gen.invokeInterface(type, m);
  ...
  static class TryExpr implements Expr {
    ...
    public void emit(C context, FnExpr fn, GeneratorAdapter gen) {
      ...
      gen.mark(startTry);
      tryExpr.emit(context, fn, gen);
      if (context != C.STATEMENT) gen.visitVarInsn(OBJECT_TYPE.getOpcode(Opcodes.ISTORE), retLocal);
      gen.mark(endTry);
      if (finallyExpr != null) finallyExpr.emit(C.STATEMENT, fn, gen);
      gen.goTo(ret);
      ...
      gen.mark(ret);
      if (context != C.STATEMENT) gen.visitVarInsn(OBJECT_TYPE.getOpcode(Opcodes.ILOAD), retLocal);
      gen.mark(end);
      for (int i = 0; i < catchExprs.count(); i++) {
        CatchClause clause = (CatchClause) catchExprs.nth(i);
        gen.visitTryCatchBlock(
            startTry, endTry, clause.label, clause.c.getName().replace('.', '/'));
      }
      if (finallyExpr != null) gen.visitTryCatchBlock(startTry, endTryCatch, finallyLabel, null);
      for (int i = 0; i < catchExprs.count(); i++) {
        CatchClause clause = (CatchClause) catchExprs.nth(i);
    ...
    static class Parser implements IParser {
      ...
      public Expr parse(C context, Object frm) throws Exception {
        ...
        for (ISeq fs = form.rest(); fs != null; fs = fs.rest()) {
          ...
          if (!Util.equal(op, CATCH) && !Util.equal(op, FINALLY)) {
            ...
            if (Util.equal(op, CATCH)) {
              ...
              if (sym.getNamespace() != null) throw new Exception("Can't bind qualified name");
        ...
        return new TryExpr(
            (new BodyExpr.Parser()).parse(context, RT.seq(body)),
            catches,
            finallyExpr,
            retLocal,
            finallyLocal);
  ...
  static class NewExpr implements Expr {
    ...
    public NewExpr(Class c, IPersistentVector args, int line) throws Exception {
      ...
      if (ctors.isEmpty()) throw new IllegalArgumentException("No matching ctor found");
      ...
      if (ctor == null && RT.booleanCast(RT.WARN_ON_REFLECTION.get())) {
        System.err.format(
            "Reflection warning, line: %d - call to %s ctor can't be resolved.\n",
            line, c.getName());
      }
    }
  ...
  static class IfExpr implements Expr {
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object frm) throws Exception {
        ...
        if (form.count() > 4) throw new Exception("Too many arguments to if");
        else if (form.count() < 3) throw new Exception("Too few arguments to if");
        return new IfExpr(
            (Integer) LINE.get(),
  ...
  static class EmptyExpr implements Expr {
    static final Type HASHMAP_TYPE = Type.getType(PersistentHashMap.class);
    ...
    public void emit(C context, FnExpr fn, GeneratorAdapter gen) {
      if (coll instanceof IPersistentList) gen.getStatic(LIST_TYPE, "EMPTY", EMPTY_LIST_TYPE);
      else if (coll instanceof IPersistentVector) gen.getStatic(VECTOR_TYPE, "EMPTY", VECTOR_TYPE);
      else if (coll instanceof IPersistentMap) gen.getStatic(HASHMAP_TYPE, "EMPTY", HASHMAP_TYPE);
  ...
  static class MapExpr implements Expr {
    ...
    public static Expr parse(C context, IPersistentMap form) throws Exception {
      ...
      for (ISeq s = RT.seq(form); s != null; s = s.rest()) {
        ...
        keyvals =
            (IPersistentVector)
                keyvals.cons(analyze(context == C.EVAL ? context : C.EXPRESSION, e.key()));
        keyvals =
            (IPersistentVector)
                keyvals.cons(analyze(context == C.EVAL ? context : C.EXPRESSION, e.val()));
      }
    ...
    public Object eval() throws Exception {
      ...
      for (int i = 0; i < keyvals.count(); i++) ret[i] = ((Expr) keyvals.nth(i)).eval();
      return RT.map(ret);
    }
  ...
  static class SetExpr implements Expr {
    ...
    public static Expr parse(C context, IPersistentSet form) throws Exception {
      ...
      for (ISeq s = RT.seq(form); s != null; s = s.rest()) {
        Object e = s.first();
        keys =
            (IPersistentVector) keys.cons(analyze(context == C.EVAL ? context : C.EXPRESSION, e));
      }
    ...
    public Object eval() throws Exception {
      ...
      for (int i = 0; i < keys.count(); i++) ret[i] = ((Expr) keys.nth(i)).eval();
      return RT.set(ret);
    }
  ...
  static class VectorExpr implements Expr {
    static final Method vectorMethod =
    ...
    public static Expr parse(C context, IPersistentVector form) throws Exception {
      ...
      for (int i = 0; i < form.count(); i++)
        args =
            (IPersistentVector)
                args.cons(analyze(context == C.EVAL ? context : C.EXPRESSION, form.nth(i)));
  ...
  static class InvokeExpr implements Expr {
    ...
    public Object eval() throws Exception {
      ...
      for (int i = 0; i < args.count(); i++) argvs = argvs.cons(((Expr) args.nth(i)).eval());
    ...
    public void emit(C context, FnExpr fn, GeneratorAdapter gen) {
      ...
      gen.invokeInterface(
          IFN_TYPE,
          new Method(
              "invoke", OBJECT_TYPE, ARG_TYPES[Math.min(MAX_POSITIONAL_ARITY + 1, args.count())]));
      if (context == C.STATEMENT) gen.pop();
    }
  ...
  public static class FnExpr implements Expr {
    ...
    static final Type aFnType = Type.getType(AFn.class);
    ...
    static Expr parse(C context, ISeq form, String name) throws Exception {
      ...
      fn.simpleName = ((name != null ? munge(name) : "fn") + "__" + RT.nextID());
      ...
      try {
        ...
        if (RT.second(form) instanceof IPersistentVector) form = RT.list(FN, RT.rest(form));
        ...
        for (ISeq s = RT.rest(form); s != null; s = RT.rest(s)) {
          ...
          if (f.isVariadic()) {
            ...
            else throw new Exception("Can't have more than 1 variadic overload");
          } else if (methodArray[f.reqParms.count()] == null) methodArray[f.reqParms.count()] = f;
          else throw new Exception("Can't have 2 overloads with same arity");
        }
        ...
        for (int i = 0; i < methodArray.length; i++)
          if (methodArray[i] != null) methods = RT.conj(methods, methodArray[i]);
    ...
    Type[] ctorTypes() {
      if (closes.count() == 0) return ARG_TYPES[0];
      PersistentVector tv = PersistentVector.EMPTY;
      for (ISeq s = RT.keys(closes); s != null; s = s.rest()) {
        LocalBinding lb = (LocalBinding) s.first();
        if (lb.getPrimitiveType() != null) tv = tv.cons(Type.getType(lb.getPrimitiveType()));
        else tv = tv.cons(OBJECT_TYPE);
      }
      Type[] ret = new Type[tv.count()];
      for (int i = 0; i < tv.count(); i++) ret[i] = (Type) tv.nth(i);
      return ret;
    }
    ...
    private void compile() {
      ...
      cv.visit(
          ...
          internalName,
          null,
          isVariadic() ? "clojure/lang/RestFn" : "clojure/lang/AFn",
          null);
      ...
      GeneratorAdapter clinitgen =
          new GeneratorAdapter(
              ACC_PUBLIC + ACC_STATIC, Method.getMethod("void <clinit> ()"), null, null, cv);
      clinitgen.visitCode();
      ...
      for (ISeq s = RT.keys(closes); s != null; s = s.rest()) {
        ...
        if (lb.getPrimitiveType() != null)
          cv.visitField(
              ...
              lb.name,
              Type.getType(lb.getPrimitiveType()).getDescriptor(),
              null,
              null);
        else
    ...
    public void emit(C context, FnExpr fn, GeneratorAdapter gen) {
      // emitting a Fn means constructing an instance, feeding closed-overs from enclosing scope, if
      // any
      ...
      for (ISeq s = RT.keys(closes); s != null; s = s.rest()) {
        LocalBinding lb = (LocalBinding) s.first();
        if (lb.getPrimitiveType() != null) fn.emitUnboxedLocal(gen, lb);
        else fn.emitLocal(gen, lb);
      }
      //		gen.invokeConstructor(fntype, new Method("<init>", Type.VOID_TYPE,
      // ARG_TYPES[closes.count()]));
    ...
    private void emitLocal(GeneratorAdapter gen, LocalBinding lb) {
      if (closes.containsKey(lb)) {
        ...
        } else gen.getField(fntype, lb.name, OBJECT_TYPE);
      } else {
        Class primc = lb.getPrimitiveType();
        if (primc != null) {
    ...
    Type constantType(int id) {
      ...
      if (Modifier.isPublic(c.getModifiers())) {
        ...
        if (RestFn.class.isAssignableFrom(c)) return Type.getType(RestFn.class);
  ...
  static class FnMethod {
    ...
    private static FnMethod parse(FnExpr fn, ISeq form) throws Exception {
      ...
      try {
        ...
        Var.pushThreadBindings(
            RT.map(
                METHOD, method, LOCAL_ENV, LOCAL_ENV.get(), LOOP_LOCALS, null, NEXT_LOCAL_NUM, 0));

        // register 'this' as local 0
        // registerLocal(THISFN, null, null);
        registerLocal(
            Symbol.intern(fn.thisName != null ? fn.thisName : "fn__" + RT.nextID()), null, null);
        ...
        for (int i = 0; i < parms.count(); i++) {
          ...
          if (p.equals(_AMP_)) {
            ...
            else throw new Exception("Invalid parameter list");
          } else {
    ...
    public void emit(FnExpr fn, ClassVisitor cv) {
      Method m =
          new Method(isVariadic() ? "doInvoke" : "invoke", OBJECT_TYPE, ARG_TYPES[numParams()]);
  ...
  static class LocalBindingExpr implements Expr, MaybePrimitiveExpr {
    ...
    public void emit(C context, FnExpr fn, GeneratorAdapter gen) {
      if (context != C.STATEMENT) fn.emitLocal(gen, b);
    }
  ...
  static class BodyExpr implements Expr {
    ...
    static class Parser implements IParser {
      public Expr parse(C context, Object frms) throws Exception {
        ...
        if (Util.equal(RT.first(forms), DO)) forms = RT.rest(forms);
        PersistentVector exprs = PersistentVector.EMPTY;
        for (; forms != null; forms = forms.rest()) {
          Expr e =
              (context != C.EVAL && (context == C.STATEMENT || forms.rest() != null))
  ...
  static class LetExpr implements Expr {
    ...
    public void emit(C context, FnExpr fn, GeneratorAdapter gen) {
      ...
      for (ISeq bis = bindingInits.seq(); bis != null; bis = bis.rest()) {
        ...
        if (primc != null)
          gen.visitLocalVariable(
              bi.binding.name, Type.getDescriptor(primc), null, loopLabel, end, bi.binding.idx);
        else
          gen.visitLocalVariable(
              bi.binding.name, "Ljava/lang/Object;", null, loopLabel, end, bi.binding.idx);
      }
    }
      ...
      public Expr parse(C context, Object frm) throws Exception {
        ...
        IPersistentMap dynamicBindings =
            RT.map(
                LOCAL_ENV, LOCAL_ENV.get(),
                NEXT_LOCAL_NUM, NEXT_LOCAL_NUM.get());
        if (isLoop) dynamicBindings = dynamicBindings.assoc(LOOP_LOCALS, null);
        ...
        try {
          ...
          for (int i = 0; i < bindings.count(); i += 2) {
            ...
            if (sym.getNamespace() != null) throw new Exception("Can't let qualified name");
          ...
          return new LetExpr(
              bindingInits,
              (new BodyExpr.Parser()).parse(isLoop ? C.RETURN : context, body),
              isLoop);
        } finally {
          Var.popThreadBindings();
        }
    ...
    public void emit(C context, FnExpr fn, GeneratorAdapter gen) {
      ...
      for (int i = 0; i < loopLocals.count(); i++) {
        ...
        if (lb.getPrimitiveType() != null) {
          Class primc = lb.getPrimitiveType();
          try {
            if (!(arg instanceof MaybePrimitiveExpr
                && arg.hasJavaClass()
                && arg.getJavaClass() == primc))
              throw new IllegalArgumentException(
                  "recur arg for primitive local: " + lb.name + " must be matching primitive");
          } catch (Exception e) {
            throw new RuntimeException(e);
          }
      ...
      for (int i = loopLocals.count() - 1; i >= 0; i--) {
        LocalBinding lb = (LocalBinding) loopLocals.nth(i);
        Class primc = lb.getPrimitiveType();
        if (primc != null) gen.visitVarInsn(Type.getType(primc).getOpcode(Opcodes.ISTORE), lb.idx);
