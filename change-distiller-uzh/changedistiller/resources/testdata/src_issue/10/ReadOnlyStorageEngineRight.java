...
public class ReadOnlyStorageEngine implements StorageEngine<ByteArray, byte[]> {
   ...
   * @param storageDir The directory in which the .data and .index files reside
   * @param numBackups The number of backups of these files to retain
   * @param numFileHandles The number of file descriptors to keep pooled for each file
   * @param bufferWaitTimeoutMs The maximum time to wait to acquire a file handle
   * @param maxCacheSizeBytes The maximum size of the cache, in bytes. The actual size of the cache
   *     will be the largest power of two lower than this number
   */
  public ReadOnlyStorageEngine(
      String name, SearchStrategy searchStrategy, File storeDir, int numBackups) {
  ...
  @JmxOperation(
      description =
          "swapFiles(newIndexFile, newDataFile) changes this store "
  ...
  public void swapFiles(String newStoreDirectory) {
    ...
    try {
      ...
      try {
        ...
        if (success)
          logger.info(
              "Swap operation completed successfully on store " + getName() + ", releasing lock.");
  ...
  private void shiftBackupsRight(int beginShift) {
    ...
    if (dest.exists()) shiftBackupsRight(beginShift + 1);
  ...
  public ClosableIterator<ByteArray> keys() {
    throw new UnsupportedOperationException(
        "Iteration is not supported for " + getClass().getName());
  }
  ...
  public List<Versioned<byte[]>> get(ByteArray key) throws VoldemortException {
    ...
    int location =
        searchStrategy.indexOf(
            fileSet.indexFileFor(chunk), keyMd5, fileSet.getIndexFileSize(chunk));
    if (location >= 0) {
  ...
  public boolean delete(ByteArray key, Version version) throws VoldemortException {
    throw new UnsupportedOperationException(
        "Delete is not supported on this store, it is read-only.");
  }
