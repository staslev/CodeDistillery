...
public class JsonStoreBuilder {
  ...
  public static void main(String[] args) throws IOException {
    ...
    try {
      JsonReader reader = new JsonReader(new BufferedReader(new FileReader(inputFile), 1000000));
      Cluster cluster =
          new ClusterMapper().readCluster(new BufferedReader(new FileReader(clusterFile)));
      StoreDefinition storeDef = null;
      List<StoreDefinition> stores =
          new StoreDefinitionsMapper()
              .readStoreList(new BufferedReader(new FileReader(storeDefFile)));
      ...
      ConsistentRoutingStrategy routingStrategy =
          new ConsistentRoutingStrategy(cluster.getNodes(), storeDef.getReplicationFactor());

      new JsonStoreBuilder(
              reader, cluster, storeDef, routingStrategy, outputDir, sortBufferSize, numThreads)
          .build();
  ...
  public void build() throws IOException {
    ...
    for (Node node : cluster.getNodes()) {
      ...
      indexes[current] =
          new DataOutputStream(new BufferedOutputStream(new FileOutputStream(indexFile), 1000000));
      datas[current] =
          new DataOutputStream(new BufferedOutputStream(new FileOutputStream(dataFile), 1000000));
      current++;
    }
    ...
    int count = 0;
    ExternalSorter<KeyValuePair> sorter =
        new ExternalSorter<KeyValuePair>(
            new KeyValuePairSerializer(), new KeyMd5Comparator(), internalSortSize, numThreads);
  ...
  private static class JsonObjectIterator extends AbstractIterator<KeyValuePair> {
    ...
    public JsonObjectIterator(
        JsonReader reader, Serializer<Object> keySerializer, Serializer<Object> valueSerializer) {
      this.reader = reader;
      ...
      try {
        ...
        try {
          ...
          throw new VoldemortException(
              "Invalid file: reached end of file with key but no matching value.");
        }
