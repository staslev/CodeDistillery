...
public class WhenConsecutiveCommentsAreJoined {
  ...
  public static void prepareCompilationUnit() throws Exception {
    sCompilationUnit =
        CompilationUtils.compileFile("src_comments/ClassWithConsecutiveComments.java");
  ...
  public void consecutiveLineCommentsShouldBeJoined() throws Exception {
    assertThat(
        getCommentString(sComments.get(0)),
        is("// a simple method invocation" + LF + "        // simple indeed"));
    assertThat(
        sComments.get(0).getComment(),
        is("// a simple method invocation" + LF + "        // simple indeed"));
  }
