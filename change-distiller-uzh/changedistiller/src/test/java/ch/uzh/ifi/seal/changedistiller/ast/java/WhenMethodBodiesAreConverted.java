...
public class WhenMethodBodiesAreConverted extends WhenASTsAreConverted {
  ...
  public void ifStatementShouldBeConverted() throws Exception {
    fSnippet =
        "if (list.isEmpty()) { System.out.print(\"empty\"); } else { System.out.print(\"not empty\"); }";
    prepareCompilation();
    convert();
    assertThat(getFirstChild().getLabel(), is(JavaEntityType.IF_STATEMENT));
    assertThat(
        ((Node) getFirstChild().getFirstChild()).getLabel(), is(JavaEntityType.THEN_STATEMENT));
    assertThat(
        ((Node) getFirstChild().getLastChild()).getLabel(), is(JavaEntityType.ELSE_STATEMENT));
    assertThat(
        getTreeString(),
        is(
  ...
  public void tryStatementShouldBeConverted() throws Exception {
    ...
    assertThat(
        ((Node) ((Node) getFirstChild().getFirstChild()).getNextSibling().getFirstChild())
            .getLabel(),
    ...
    assertThat(
        getTreeString(),
        is(
            "method {  {  { foo.bar(e); }, { IOException { 2; },Exception { 3; } }, { cleanup(); } } }"));
    assertSourceRangeCorrectness(getFirstChild());
  }
  ...
  public void tryStatementWithoutFinallyShouldBeConverted() throws Exception {
    fSnippet =
        "try { foo.bar(e); } catch (IOException e) { return 2; } catch (Exception e) { return 3; }";
    prepareCompilation();
    convert();
    assertThat(
        ((Node) ((Node) getFirstChild().getFirstChild()).getNextSibling()).getLabel(),
    ...
    assertThat(
        getTreeString(),
        is("method {  {  { foo.bar(e); }, { IOException { 2; },Exception { 3; } } } }"));
    assertSourceRangeCorrectness(getFirstChild());
  }
  ...
  private void assertSourceRangeCorrectness(Node node) {
    ...
    String source =
        fCompilation.getSource().substring(entity.getStartPosition(), entity.getEndPosition() + 1);
  ...
  private void convert() {
    ...
    AbstractMethodDeclaration method =
        CompilationUtils.findMethod(fCompilation.getCompilationUnit(), "method");
