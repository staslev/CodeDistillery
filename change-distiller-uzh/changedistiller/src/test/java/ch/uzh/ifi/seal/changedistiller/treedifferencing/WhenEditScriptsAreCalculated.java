...
public class WhenEditScriptsAreCalculated extends TreeDifferencingTestCase {
  ...
  public void insertShouldWork() throws Exception {
    ...
    Node innerTryRight = addToNode(outerTryRight, JavaEntityType.TRY_STATEMENT, "");
    createEditScript();
    ...
    assertThat(
        ((Node) insert.getNodeToInsert().getParent()).getLabel(), is(innerTryRight.getLabel()));
    assertThat(
        ((Node) insert.getNodeToInsert().getParent()).getValue(), is(innerTryRight.getValue()));
  }
