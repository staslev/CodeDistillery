...
public class CommentRecorderParser extends Parser {
  ...
  public void checkComment() {
    ...
    nextComment:
    for (lastCommentIndex = scanner.commentPtr; lastCommentIndex >= 0; lastCommentIndex--) {
      ...
      if (javadocParser.shouldReportProblems) {
        javadocParser.reportProblems =
            (currentElement == null) || (commentSourceEnd > lastJavadocEnd);
      } else {
  ...
  public int flushCommentsDefinedPriorTo(int position) {
    ...
    if (validCount > 0) {
      ...
      while ((index < lastCommentIndex)
          && ((immediateCommentEnd = -scanner.commentStops[index + 1]) > 0)) { // only
        // tolerating
        // non-javadoc
        // comments
        // (non-javadoc
        ...
        if (org.eclipse.jdt.internal.compiler.util.Util.getLineNumber(
                position, scanner.lineEnds, 0, scanner.linePtr)
            != org.eclipse.jdt.internal.compiler.util.Util.getLineNumber(
                immediateCommentEnd, scanner.lineEnds, 0, scanner.linePtr)) {
          break;
        }
    ...
    switch (validCount) {
      ...
      default:
        ...
        System.arraycopy(
            scanner.commentTagStarts, index + 1, scanner.commentTagStarts, 0, validCount);
    }
  ...
  public void initializeScanner() {
    scanner =
        new Scanner(
            false /*comment*/,
            false /*whitespace*/,
            options.getSeverity(CompilerOptions.NonExternalizedString)
                != ProblemSeverities.Ignore /*nls*/,
  ...
  private void pushOnCommentsStack(int start, int end) {
    ...
    for (int i = start; i <= end; i++) {
      ...
      int scannerStart =
          scanner.commentStarts[i] < 0 ? -scanner.commentStarts[i] : scanner.commentStarts[i];
      int commentStart =
          commentPtr == -1
              ? -1
              : (commentStarts[commentPtr] < 0
