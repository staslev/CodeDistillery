 ...
 *
 * <p>The implementation style is not clean, i.e., it uses single character method and variable
 * names, because the implementation should reflect the original algorithm from the paper as much as
 * possible.
 *
 * @author Beat Fluri
...
public class TreeDifferencer {
  ...
  private boolean matchContains(Node v, Node y, Set<NodePair> match) {
    for (NodePair p : match) {
      if (((p.getLeft() == v) && (p.getRight() == y))
          || ((p.getLeft() == y) && (p.getRight() == v))) {
        return true;
      }
    }
    return false;
  ...
  private void extractLCS(
      int[][] b, List<Node> l, List<Node> r, int i, int j, HashSet<NodePair> lcs) {
    if ((i != 0) && (j != 0)) {
      if (b[i][j] == DIAG) {
