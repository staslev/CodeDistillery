...
public class CommentCleaner {
  ...
  private static Pattern sSourceCodePattern =
      Pattern.compile(
          "(?s).*([aA-zZ0-9]*\\.)*[aA-zZ0-9]+?\\((?s).*?\\);(?s).*"
              + "| (?s).+=(?s).+?;(?s).*"
              + "| (?s).*if\\s*\\((?s).*\\)\\s*\\{*\\s*(?s).*"
  ...
  private void prepareConsecutiveComments(Comment comment) {
    ...
    if (previousCommentExistsAndIsLineComment()
      ...
      // If only whitespace is between previous comment and current comment, the comments are in the
      // same block
  ...
  // Uses regex patterns to guess whether a comment is commented source code and - if this is the
  // case - removes it
