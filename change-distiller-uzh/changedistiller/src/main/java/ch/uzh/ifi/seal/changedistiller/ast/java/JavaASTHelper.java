...
public class JavaASTHelper implements ASTHelper<JavaStructureNode> {
  ...
  JavaASTHelper(
    ...
    fCompilation =
        JavaCompilationUtils.compile(content.getContent(), content.getName(), versionNumber);
    prepareComments();
  ...
  private List<Comment> collectComments() {
    CommentCollector collector =
        new CommentCollector(fCompilation.getCompilationUnit(), fCompilation.getSource());
    collector.collect();
  ...
  public Node createDeclarationTree(JavaStructureNode node, String qualifiedName) {
    Node root = createDeclarationTree(node);
  ...
  public StructureEntityVersion createStructureEntityVersion(JavaStructureNode node) {
    return new StructureEntityVersion(
        convertType(node), node.getFullyQualifiedName(), extractModifier(node.getASTNode()));
  }
  ...
  public StructureEntityVersion createMethodInClassHistory(
      ClassHistory classHistory, JavaStructureNode node, String versionNum) {
    MethodHistory mh;
