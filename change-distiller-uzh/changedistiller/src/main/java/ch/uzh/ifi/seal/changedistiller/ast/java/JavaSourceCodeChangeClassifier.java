...
public class JavaSourceCodeChangeClassifier implements SourceCodeChangeClassifier {
  ...
  private SourceCodeChange handleMethodSignatureChange(Delete delete) {
    if (delete.getChangedEntity().getType() == JavaEntityType.JAVADOC) {
      ...
      if (delete.getChangedEntity().getType().isType()) {
        // whenever void as return type is deleted, a concrete return type was inserted. therefore
        // we can ignore
        // this delete change
  ...
  private boolean isEqual(
      SourceCodeEntity entity, EntityType expectedEntityType, String expectedEntityName) {
    boolean type = false;
    boolean name = false;
  ...
  private boolean isEqual(
      StructureEntityVersion entity, EntityType expectedEntityType, String expectedEntityName) {
    boolean type = false;
    boolean name = false;
    if (expectedEntityType == null) {
      type = true;
