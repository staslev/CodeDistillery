...
public class ClassDistiller {
  ...
  public void extractChanges() {
    ...
    for (final StructureDiffNode diffNode : fClassDiffNode.getChildren()) {
      processDiffNode(diffNode);
    }

    if (fVersion != null) {
      fRefactoringProcessor.processRefactoringCandidates(
          fClassHistory, fLeftASTHelper, fRightASTHelper, fRefactoringContainer, fVersion);
    } else {
      fRefactoringProcessor.processRefactoringCandidates(
          fClassHistory, fLeftASTHelper, fRightASTHelper, fRefactoringContainer);
    }
  ...
  private void processDiffNode(final StructureDiffNode diffNode) {
  ...
  private void processClassDiffNode(StructureDiffNode diffNode) {
    ...
    if (fVersion != null) {
      ClassHistory classHistory =
          fClassHistory.createInnerClassHistory(
      ...
      ClassHistory classHistory =
          fClassHistory.createInnerClassHistory(
              fLeftASTHelper.createStructureEntityVersion(diffNode.getLeft()));
      classDistiller =
          new ClassDistiller(
              diffNode,
              classHistory,
  ...
  private void processChanges(final StructureDiffNode diffNode) {
    ...
    // get the inner changes hiding inside the children diff-nodes
    for (final StructureDiffNode structureDiffNode : diffNode.getChildren()) {
  ...
  private StructureEntityVersion createFieldStructureEntity(StructureDiffNode diffNode) {
    if (fVersion != null) {
      return fRightASTHelper.createFieldInClassHistory(
          fClassHistory, diffNode.getRight(), fVersion);
    } else {
  ...
  private StructureEntityVersion createMethodStructureEntity(StructureDiffNode diffNode) {
    if (fVersion != null) {
      return fRightASTHelper.createMethodInClassHistory(
          fClassHistory, diffNode.getRight(), fVersion);
    } else {
  ...
  private void processDeclarationChanges(
      StructureDiffNode diffNode, StructureEntityVersion rootEntity) {
    extractChanges(
