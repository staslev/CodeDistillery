 ...
 *
 * <p>Whenever the {@link RefactoringExtractor} decides that two candidates belong to a refactoring,
 * it generates a refactoring pair.
 *
 * @author fluri
...
public class RefactoringPair implements Comparable<RefactoringPair> {
   ...
   */
  public RefactoringPair(
      RefactoringCandidate deletedEntity, RefactoringCandidate insertedEntity, double similarity) {
  ...
  public int compareTo(RefactoringPair other) {
    // Double.compare sorts ascending, we'd like to have it descending, so we return the negative
    // result
