...
public final class JavaCompilationUtils {
   ...
   */
  public static JavaCompilation compile(String source, String sourceName, Long langVersion) {
    final CompilerOptions options;
    if (langVersion != null) {
      options = getDefaultCompilerOptions(langVersion);
    } else {
      options = getDefaultCompilerOptions();
    }
    Parser parser = createCommentRecorderParser(options);
    ICompilationUnit cu = createCompilationUnit(source, sourceName);
    CompilationResult compilationResult = createDefaultCompilationResult(cu, options);
    JavaCompilation javaCompilation =
        new JavaCompilation(parser.parse(cu, compilationResult), parser.scanner);

    if (compilationResult.hasSyntaxError) {
      throw new InvalidSyntaxException(
          new String(compilationResult.getFileName()), compilationResult.toString());
    }

    return javaCompilation;
  }

  public static JavaCompilation compile(String content, String contentName) {
    return compile(content, contentName, null);
  }
  ...
  private static Parser createCommentRecorderParser(CompilerOptions options) {
    return new CommentRecorderParser(
        new ProblemReporter(
