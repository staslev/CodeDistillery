...
public class JavaMethodBodyConverter extends ASTVisitor {
  ...
  public void preVisit(ASTNode node) {
    ...
    while (i < fComments.size()) {
      ...
      if (previousNodeExistsAndIsNotTheFirstNode()
          && isCommentBetweenCurrentNodeAndLastNode(comment, node)) {
   ...
   * comment.
   *
   * @param left to calculate the proximity
   * @param right to calculate the proximity
   * @return <code>2</code> if the comment node is on the same line as the other node, <code>1
   *     </code> if they are on adjacent line, <code>0</code> otherwise (times two)
   */
  ...
  public boolean visit(
      QualifiedAllocationExpression qualifiedAllocationExpression, BlockScope scope) {
  ...
  public boolean visit(BreakStatement breakStatement, BlockScope scope) {
    ...
    pushValuedNode(
        breakStatement, breakStatement.label != null ? String.valueOf(breakStatement.label) : "");
    return false;
  }
  ...
  public boolean visit(ForeachStatement foreachStatement, BlockScope scope) {
    ...
    pushValuedNode(
        foreachStatement,
        foreachStatement.elementVariable.printAsExpression(0, new StringBuffer()).toString()
            + COLON
   ...
   * @return <code>true</code> if the children of the expression should be visited, <code>false
   *     </code> otherwise.
   */
  ...
  public boolean visit(ReturnStatement returnStatement, BlockScope scope) {
    ...
    pushValuedNode(
        returnStatement,
        returnStatement.expression != null ? returnStatement.expression.toString() + ';' : "");
    return false;
  }
  ...
  public boolean visit(CaseStatement caseStatement, BlockScope scope) {
    ...
    pushValuedNode(
        caseStatement,
        caseStatement.constantExpression != null
            ? caseStatement.constantExpression.toString()
            : "default");
    return false;
  }
  ...
  private void visitFinally(TryStatement node, BlockScope scope) {
    if (node.finallyBlock != null) {
      push(
          JavaEntityType.FINALLY,
          "",
          node.finallyBlock.sourceStart(),
    ...
    if ((node.catchBlocks != null) && (node.catchBlocks.length > 0)) {
      ...
      for (int i = 0; i < node.catchArguments.length; i++) {
        int catchClauseSourceStart =
            retrieveStartingCatchPosition(start, node.catchArguments[i].sourceStart);
        push(
