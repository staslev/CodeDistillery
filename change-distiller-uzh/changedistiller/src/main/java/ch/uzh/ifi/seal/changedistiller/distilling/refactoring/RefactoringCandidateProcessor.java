...
public class RefactoringCandidateProcessor {
  ...
  @Inject
  RefactoringCandidateProcessor(
      DistillerFactory distillerFactory, SourceCodeChangeClassifier changeClassifier) {
  ...
  private void processFieldRefactoringCandidates(RefactoringCandidateContainer candidates) {
    checkRefactorings(
        candidates.getAddedFields(),
        candidates.getDeletedFields(),
        new FieldRefactoringHelper(fClassHistory, fRightASTHelper));
  }

  private void processMethodRefactoringCandidates(RefactoringCandidateContainer candidates) {
    checkRefactorings(
        candidates.getAddedMethods(),
        candidates.getDeletedMethods(),
        new MethodRefactoringHelper(fClassHistory, fRightASTHelper));
  }
    ...
    for (RefactoringPair pair : refactorings) {
      ...
      if (fVersion != null) {
        structureEntityVersion =
            refactoringHelper.createStructureEntityVersionWithID(rightDrn, fVersion);
      } else {
      ...
      // use the new qualified name for the method; otherwise TreeDifferencer detects a return type
      // change
      ...
      if (isRenaming(leftDrn, rightDrn, refactoringHelper)) {
        if (fVersion != null) {
          structureEntityVersion =
              refactoringHelper.createStructureEntityVersionWithID(
                  leftDrn, rightDrn.getFullyQualifiedName(), fVersion);
        } else {
          structureEntityVersion =
              refactoringHelper.createStructureEntityVersion(
                  leftDrn, rightDrn.getFullyQualifiedName());
        }
        Update upd =
            new Update(
                structureEntityVersion,
                ...
                leftRoot.getEntity());
        structureEntityVersion.addAllSourceCodeChanges(
            fChangeClassifier.classifySourceCodeChanges(Arrays.asList(upd)));
      }
  ...
  private boolean isRenaming(
      StructureNode leftDrn, StructureNode rightDrn, AbstractRefactoringHelper refactoringHelper) {
  ...
  private void processRemainingDiffs(
      StructureEntityVersion clazz, List<RefactoringCandidate> candidates) {
    for (RefactoringCandidate candidate : candidates) {
      if (!candidate.isRefactoring()) {
        List<SourceCodeChange> classifiedChanges =
            fChangeClassifier.classifySourceCodeChanges(
                Arrays.asList(candidate.getSourceCodeChange()));
