...
public class JavaStructureTreeBuilder extends ASTVisitor {
  ...
  public boolean visit(
      CompilationUnitDeclaration compilationUnitDeclaration, CompilationUnitScope scope) {
  ...
  private void push(Type type, String name, ASTNode astNode) {
    ...
    final String prefix = hasTestAnnotation(type, astNode) ? "@Test@." : "";

    JavaStructureNode node =
        new JavaStructureNode(type, qualifier == null ? null : prefix + qualifier, name, astNode);
    fNodeStack.peek().addChild(node);
    fNodeStack.push(node);
  }

  private boolean hasTestAnnotation(final Type type, final ASTNode astNode) {
    if (type.equals(Type.METHOD) && astNode instanceof MethodDeclaration) {
      final MethodDeclaration methodDeclaration = (MethodDeclaration) astNode;
      if (methodDeclaration.annotations != null) {
        for (final Annotation annotation : methodDeclaration.annotations) {
          if (annotation.type.toString().equals("Test")) {
            return true;
          }
        }
      }
